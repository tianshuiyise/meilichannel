...
<!-- 检索条件 -->
<form id="userForm" action="${ctx }/user/modifyUser">
	<!-- 列表区 -->
	<table id="table1">
		<tr id="tr1">
	    	...
	    	<td>
	    		<select id="mul_roleNames" name="roleNames" multiple="multiple" size="5">
    				<c:forEach items="${roleList }" var="role">
						<option value="${role.roleKey }" <c:if test="${role.selected }">selected="selected"</c:if>>${role.roleName }</option>
					</c:forEach>
	    		</select>
	    	</td>
	    	<td id="right"><font color="red">*</font>用户类型</td>
	    	<td>
	    		<!-- <select id="mul_userTypes" name="userTypes" multiple="multiple" size="5">
    				<option value="1">管理员</option>
					<option value="2">教师</option>
					<option value="4">学生</option>
	    		</select> -->
	    		<tags:wmultipleselect name="userTypes" key="UserType" value="${user.userType }"></tags:wmultipleselect>
	    	</td>
	    </tr>
	    <tr>
	    	<td id="right">备注</td>
	        <td colspan="3" id="left"><textarea id="txt_remark" name="remark" rows="3" cols="40">${user.remark }</textarea></td>
	    </tr>
	</table>
